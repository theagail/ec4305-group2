...
function [theta,LL] = copulafit_myown(family,u)
...
switch family
    ...
    case 'gaussian'
        ...
        [theta,LL] = fminbnd(fun,-1,1,optimset('Display','off'));
    case 't'
        fun = @(theta) -sum(log(copulapdf(family,u,[1 theta(1);theta(1) 1],theta(2))+eps));
        init = [0; 1];
        lb = [-1; 1e-1];
        ub = [1; 1000];
        options = optimset('Algorithm','interior-point','Display','off');
        [theta,LL] = fmincon(fun,init,[],[],[],[],lb,ub,[],options);
    case {'clayton','claytonrot090','claytonrot180','claytonrot270'}
        fun = @(theta) -sum(log(copulapdf(family,u,theta)+eps));
        [theta,LL] = fminbnd(fun,1e-3,20,optimset('Display','off'));
    otherwise
